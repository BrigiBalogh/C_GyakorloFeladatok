...
namespace NagyzhPelda3
{
    internal class Program
    {
        ...
        static void Main()
        {
            List<Coupon> coupons = LoadCoupons("coupons1.json");
            List<Product> products = Utils.LoadFromJson<Product>("purchases.json");


        public static List<Coupon> LoadCoupons(string filename)
        {
            var rawCoupons = Utils.LoadFromJson<CouponRaw>(filename);
            List<Coupon> coupons = new();


            if (rawCoupons != null)
            {
                foreach (var raw in rawCoupons)
                {
                    Coupon? coupon = raw.Type.ToLower() switch
                    {
                        "Single use" => new SingleUseCoupon(raw.Value),
                        "Multiple use" => new MultipleUseCoupon(raw.Value, raw.TotalUses),
                        "Monthly" => new MonthlyCoupon(raw.Value, raw.Year, raw.Month),
                        _ => null
                    };
